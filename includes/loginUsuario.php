<?php
    ...
    if(!empty($_POST)) {
        if (empty($_POST['login']) || empty($_POST['pass'])) {
            ...
            $sql = 'SELECT * FROM usuarios as u INNER JOIN rol as r ON u.rol= r.rol_id WHERE usuario = ? AND u.estado != 0';
            ...
            if ($query->rowCount() > 0) {
                if (password_verify($pass, $result['clave'])) {
                    ...
                    echo '<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"></button>Usuario inactivo</div>';
                }
                    
                }else {
                    echo '<div class="alert alert-successs"><button type="button" class="close" data-dismiss="alert"></button>Redirecting</div>';
                }
            }else{ 
